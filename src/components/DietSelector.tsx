...
interface DietSelectorProps {
  ...
  options: DietOption[];
  onChange: (diet: DietOption) => void;
}

const DietSelector = ({ selectedDiet, options, onChange }: DietSelectorProps) => {
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 z-10 glass rounded-lg py-2 min-w-[180px] animate-fade-in">
          {options.map((option) => (
            <button
              key={option.id}
              className={`w-full text-left px-4 py-2 transition-all hover:bg-white/10 ${selectedDiet.id === option.id ? 'text-accent' : 'text-foreground/80'}`}
              onClick={() => {
                onChange(option);
                setIsOpen(false);
              }}
